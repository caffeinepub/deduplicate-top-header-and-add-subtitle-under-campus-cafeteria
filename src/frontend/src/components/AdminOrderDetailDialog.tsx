...
import { useUpdateOrderStatus, useUpdatePaymentStatus } from '../hooks/useQueries';
...
export default function AdminOrderDetailDialog({ order, open, onOpenChange }: AdminOrderDetailDialogProps) {
  ...
  const handleMarkAsReceived = async () => {
    ...
    setIsUpdating(true);
    try {
      await updatePaymentStatus.mutateAsync({ orderId: order.id, paymentStatus: 'Paid' });
      setTimeout(() => setIsUpdating(false), 600);
    } catch (error) {
      setIsUpdating(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-2xl">Order Details & Management</DialogTitle>
        </DialogHeader>

        <ScrollArea className="max-h-[70vh]">
          <div className="space-y-6 pr-4">
            {/* Order Header */}
            <div className={`rounded-lg bg-gradient-to-r from-orange-50 to-amber-50 p-4 transition-all ${
