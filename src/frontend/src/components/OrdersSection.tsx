...
import { Clock, CheckCircle2, ChefHat, Package, Eye } from 'lucide-react';
...
const STATUS_CONFIG = {
  ...
  Ready: { icon: CheckCircle2, color: 'bg-green-100 text-green-800', label: 'Ready for Pickup' },
...
export default function OrdersSection({ orders: initialOrders }: OrdersSectionProps) {
  ...
  const [selectedOrder, setSelectedOrder] = useState<DemoOrderType | null>(null);
  ...
  useEffect(() => {
    const unsubscribe = subscribeToOrderUpdates((updatedOrders) => {
      ...
      updatedOrders.forEach((updatedOrder) => {
        ...
        if (existingOrder && (existingOrder.status !== updatedOrder.status || existingOrder.paymentStatus !== updatedOrder.paymentStatus)) {
    ...
    <>
      <div className="space-y-4">
        <div>
          ...
          <p className="text-muted-foreground">Track your order status and payment</p>
        </div>
              ...
              return (
                <Card 
                  ...
                  className={`transition-all duration-500 ${
                    isUpdating ? 'ring-2 ring-orange-400 shadow-lg scale-[1.02]' : ''
                  }`}
                >
                  <CardHeader>
