...
import { getCartItems, clearCart } from './cartStorage';
...
export function subscribeToOrderUpdates(listener: OrderUpdateListener): () => void {
...
export function updateDemoOrderStatus(orderId: string, status: 'Pending' | 'Preparing' | 'Ready' | 'Completed') {
  const orders = getDemoOrders();
...
export function updateDemoPaymentStatus(orderId: string, paymentStatus: 'Pending' | 'Paid') {
  const orders = getDemoOrders();
  const orderIndex = orders.findIndex(o => o.id === orderId);
...
// Add demo food item
export function addDemoFoodItem(name: string, description: string, price: bigint, category: string): string {
...
export function updateDemoFoodItem(id: string, name: string, description: string, price: bigint, category: string) {
  const items = getDemoMenuItems();
